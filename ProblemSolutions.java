 ...
 ********************************************************************/
 ...
 class ProblemSolutions {
     ...
     public boolean canFinish(int numExams, 
        ...
        // start from a node, see what prerequisites it needs, traverse to those nodes (IF POSSIBLE) & check those prerequisites
        // 
     ...
     public int numGroups(int[][] adjMatrix) {
         ...
         for (int x = 0; x < numNodes; x++) {
             if (!visited[x]) {
                // searches through the entirety of a group of vertices, incrementing groups
                // by one after one whole group is searched through
                 dfs(x, graph, visited);
                 groups++;
             }
         }
